...
// POST /api/v1/nfc/request
// Called by RECEIVER: "I want to receive X ETH from whoever taps me"
exports.createRequest = async (req, res) => {
  ...
  try {
    ...
    if (!user?.wallet_address) {
      return res.status(400).json({ error: 'No wallet linked to your account', code: 'WALLET_NOT_FOUND' });
    }

    const [request] = await db('nfc_requests')
      .insert({
    ...
    return res.status(201).json({
      ...
      // This payload goes into the NFC tag on the receiver's phone
      nfcPayload: `chainpay://nfc?requestId=${request.id}&to=${request.to_address}&amount=${request.amount_eth}`,
    });
  } catch (err) {
    console.error('NFC create request error:', err.message);
    return res.status(500).json({ error: 'Failed to create NFC request', code: 'INTERNAL_ERROR' });
  }
};

// POST /api/v1/nfc/confirm
// Called by SENDER after MetaMask signs: records the tx
exports.confirmRequest = async (req, res) => {
  ...
  try {
    ...
    if (request.status === 'completed') {
      return res.status(409).json({ error: 'Request already completed', code: 'ALREADY_COMPLETED' });
    }
    ...
    // Notify receiver via SSE if available
    try {
      ...
      if (receiver && sendToUser) {
        sendToUser(receiver.id, 'nfc_payment_received', {
          txHash,
          fromAddress,
          amountEth: request.amount_eth,
        });
    ...
    return res.status(500).json({ error: 'Failed to confirm NFC payment', code: 'INTERNAL_ERROR' });
  }
};

// GET /api/v1/nfc/request/:requestId
// Sender polls this to get request details before paying
